    ...
    def _step(self, batch, mode="train"):
        if mode != "predict":
            if self.args.use_prompt:
                ...
                input_ids, attention_mask, labels, images, aux_imgs = batch
            ...
            outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, images=images, aux_imgs=aux_imgs)
    ...
    def _save_results(self, dataloader, mode='train'):
        ...
        with open(result_path, 'w', encoding='utf-8') as f:
            for i in range(len(words)):
                ...
                f.write('\n')
        subset = '训练' if mode == 'train' else '测试' if mode == 'test' else '验证'
        print(f"{subset}集文件输出至{result_dir}目录下{os.path.basename(dst_file)}文件")
...
class NERTrainer(BaseTrainer):
    ...
    def _step(self, batch, mode="train"):
        ...
        output = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, images=images, aux_imgs=aux_imgs)
