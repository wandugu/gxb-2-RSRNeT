...
class HMNeTREModel(nn.Module):
    def __init__(self, num_labels, tokenizer, args):
        ...
        self.bert = RobertaModel.from_pretrained(self.args.bert_name, local_files_only=True)
    ...
    def forward(
        ...
        for i in range(bsz):
            a,b = input_ids[i].eq(self.head_start).nonzero()
            c = input_ids[i].eq(self.tail_start).nonzero()
            head_idx = a.item()
            # head_idx = input_ids[i].eq(self.head_start).nonzero().item()
            tail_idx = input_ids[i].eq(self.tail_start).nonzero().item()
            head_hidden = last_hidden_state[i, head_idx, :].squeeze()
    ...
    def get_visual_prompt(self, images, aux_imgs):
        ...
        for idx in range(12):  # 12
            ...
            prompt_gate = F.softmax(F.selu(self.gates[idx](sum_prompt_guids)), dim=-1)
            ...
            for split_aux_prompt_guid in split_aux_prompt_guids:
                ...
                # aux_prompt_gate = F.softmax(F.selu(self.gates[idx](sum_aux_prompt_guids)), dim=-1)
...
class HMNeTNERModel(nn.Module):
    def __init__(self, label_list, args):
        ...
        # self.bert = XLMRModel.from_pretrained('/root/CHB/HVPNeT-main1 (copy)/xlmr.base', checkpoint_file='model.pt')
        # self.bert.eval()  # disable dropout (or leave in train mode to finetune)
        # self.bert_config = self.bert.config
        self.bert_config = self.bert.config
        if args.use_prompt:
    ...
    def get_visual_prompt(self, images, aux_imgs):
        ...
        for idx in range(12):  # 12
            ...
            for split_aux_prompt_guid in split_aux_prompt_guids:
                sum_aux_prompt_guids = torch.stack(split_aux_prompt_guid).sum(0).view(bsz, -1) / 4     # bsz, 4, 768*2
                aux_prompt_gate = F.softmax(F.leaky_relu(self.gates[idx](sum_aux_prompt_guids)), dim=-1)
                # aux_prompt_gate = F.softmax(F.elu(self.gates[idx](sum_aux_prompt_guids)), dim=-1)
                aux_key_val = torch.zeros_like(split_aux_prompt_guid[0]).to(self.args.device)  # bsz, 4, 768*2
                for i in range(4):
                    aux_key_val = aux_key_val + torch.einsum('bg,blh->blh', aux_prompt_gate[:, i].view(-1, 1), split_aux_prompt_guid[i])
                aux_key_vals.append(aux_key_val)
...
#         aux_prompt_guids = [torch.cat(aux_prompt_guid, dim=1).view(bsz, self.args.prompt_len, -1) for aux_prompt_guid
#                             in aux_prompt_guids]
# 
...
# 
#         prompt_guids = self.gnn(prompt_guids)  # Apply GNN to learn modality correlation and weights
# 
#         aux_prompt_guids_gnn = []
#         for aux_prompt_guid in aux_prompt_guids:
#             aux_prompt_guid = self.gnn(aux_prompt_guid)  # Apply GNN to learn modality correlation and weights
#             aux_prompt_guids_gnn.append(aux_prompt_guid)
# 
#         result = []
#         for idx in range(12):
#             sum_prompt_guids = torch.stack(prompt_guids).sum(0).view(bsz, -1) / 4
#             prompt_gate = F.softmax(F.leaky_relu(self.gates[idx](sum_prompt_guids)), dim=-1)
# 
#             key_val = torch.zeros_like(prompt_guids[0]).to(self.args.device)
#             for i in range(4):
#                 key_val = key_val + torch.einsum('bg,blh->blh', prompt_gate[:, i].view(-1, 1), prompt_guids[i])
# 
#             aux_key_vals = []
#             for aux_prompt_guid in aux_prompt_guids_gnn:
#                 sum_aux_prompt_guids = torch.stack(aux_prompt_guid).sum(0).view(bsz, -1) / 4
#                 aux_prompt_gate = F.softmax(F.leaky_relu(self.gates[idx](sum_aux_prompt_guids)), dim=-1)
#                 aux_key_val = torch.zeros_like(aux_prompt_guid[0]).to(self.args.device)
#                 for i in range(4):
